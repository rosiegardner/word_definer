...
# Get the detail for a specific word such as lyrics and songwriters.
get('/words/:id/definition/:define_id') do
  @definition = Define.find(params[:define_id].to_i())
  erb(:definition)
end

# Post a new definition. After the song is added, Sinatra will route to the view for the album the song belongs to.
...
# Edit a definition and then route back to the album view.
...
# Delete a definition and then route back to the album view.
